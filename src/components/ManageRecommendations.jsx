...
export default function ManageRecommendations() {
  ...
  const fetchData = async () => {
    try {
      ...
      // Fetch all books
      const booksResponse = await fetch('http://localhost:3000/api/books/all');
  ...
  const addRecommendation = async () => {
    ...
    try {
      const response = await fetch('http://localhost:3000/api/recommendations/add', {
        method: 'POST',
        headers: {
  ...
  const removeRecommendation = async (bookId) => {
    ...
    try {
      const response = await fetch(`http://localhost:3000/api/recommendations/remove/${bookId}`, {
        method: 'DELETE'
      });
